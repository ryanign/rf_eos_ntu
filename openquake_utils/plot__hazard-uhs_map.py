...
Ryan Pranantyo
EOS, 21 April 2026
Basic plotting to check Uniform Hazard Spectra (UHS) hazard_uhs-mean.csv resulted

usage:
    python plot__hazard-uhs_map.py --input_file path_to__hazard_uhs-mean_<ID>.csv

    OR

    python plot__hazard-uhs_map.py --input_file path_to__quantile_uhs-<IMT>_<ID>.csv
...
def classic_psha(args):
    ...
    print(f'='*60)
    print(f' Basic plotting of hazard UHS values ')
    ...
    for ii, var in enumerate(variables):
        var_arr = var.split('~')
        title = f'{var_arr[1]} at {float(var_arr[0])*100}% PoE (g)'
...
if __name__ == '__main__':
   ...
   parser.add_argument(
           '--input_file',
           default='/home/ignatius.pranantyo/PSHA/Singapore/Replication_DuPan2020/case02__BackgroundSourceClassicalPSHA/hasil_case02_a/hazard_uhs-mean_3.csv',
